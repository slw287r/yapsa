...
#'
#' @param in_df A big dataframe constructed from a vcf-like file of a whole
...
#'   information.
#' @param in_keep.extra.columns in_seqinfo Argument passed on to
#'   \code{\link{makeGRangesFromDataFrame}}
#' @param in_seqinfo A seqInfo object, referring to the reference genome used.
#'   Argument passed on to \code{\link{makeGRangesFromDataFrame}}
#' @param in_seqnames.field Indicates the name of the column in which the
#'   chromosome is encoded
#' @param in_start.field Indicates the name of the column in which the start
#'   coordinate is encoded
#' @param in_end.field Indicates the name of the column in which the end
#'   coordinate is encoded
#' @param in_PID.field Indicates the name of the column in which the PID
#'   (patient or sample identifier) is encoded
#' @param in_subgroup.field Indicates the name of the column in which the
#'   subgroup information is encoded
#' @param in_strand.field Indicates the name of the column in which the
#'   strandedness is encoded
#' @param verbose_flag Verbose if 1
#'
#' @return The constructed VRanges object
#'
#' @examples
#'  data(lymphoma_test)
...
makeVRangesFromDataFrame <- function(in_df,in_keep.extra.columns=TRUE,
    ...
    if(length(which(match_list %in% name_list)) == length(match_list)) {
        ...
        if(!("+" %in% unique(strand(my_gr))) & 
                !("-" %in% unique(strand(my_gr)))){
                strand(my_gr) <- "+"
                if(verbose_flag==1){
    ...
    if(tolower(in_PID.field) %in% name_list) {
        ...
        sampleNames(out_vr) <- mcols(my_gr)[,column_ind]
            ...
            if(verbose_flag==1){
                cat(paste0(
                "YAPSA:::makeVRangesFromDataFrame::warning:in_PID.field ",
    ...
    mcols(out_vr)[,in_subgroup.field] <- my_gr$subgroup
        ...
        if(verbose_flag==1){
            cat("YAPSA:::makeVRangesFromDataFrame::warning:",
    ...
    seqlengths(out_vr) <- seqlengths(my_gr)    
        ...
        if(verbose_flag==1){
            cat("YAPSA:::makeVRangesFromDataFrame::
                error:mismatch in column names, ","return NULL.\n")
        }
    }
    return(out_vr)
}
...
save_mutation_catalogue <- function(in_table,in_matrix,in_frame,
    ...
    if (dim(type_df)[1] != length(colnames(in_matrix))) {
        cat("YAPSA:::save_mutation_catalogue::Warning: 
            mismatch between subgroup",
    ...
    if (in_subgroup_flag==2) {
        subgroup_vector <- 
            as.character(type_df$subgroup[match(colnames(in_matrix),
                                                type_df$PID)])
    }
    # store to text files
...
stratify_and_create_mutational_catalogue <- 
    ...
    for (i in seq_len(number_of_strata)) {
        if(in_verbose == 1) {
            cat("\nYAPSA:::stratify_and_create_mutational_catalogue::
        ...
        if(!is.null(strata_target_dir)) {
            ...
            save_mutation_catalogue(temp_table,temp_matrix,temp_frame,
                                    2,NULL,MotifMatrix_dir,in_verbose)
        }
